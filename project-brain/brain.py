...
INDEX_MASTER = "knowledge/indexes/INDEX-MASTER.md"
HASH_MANIFEST = ".content-hashes.json"

FILE_TYPES = {
    "SPEC":  {"dir": "identity",    "space": "identity",  "purpose": "Design decisions, architecture"},
    "CODE":  {"dir": "knowledge",   "space": "knowledge", "purpose": "Implementation"},
    "RULE":  {"dir": "identity",    "space": "identity",  "purpose": "Business/trading rules and their evolution"},
    "LEARN": {"dir": "knowledge",   "space": "knowledge", "purpose": "Discovered knowledge (edge cases, gotchas)"},
    "LOG":   {"dir": "ops",         "space": "ops",       "purpose": "Decision rationale, debugging history"},
    "RESET": {"dir": "reset-files", "space": "ops",       "purpose": "Curated context packages for specific tasks"},
}
...
def cmd_init(args):
    ...
    1. Read `knowledge/indexes/INDEX-MASTER.md`. It contains a fat index of every knowledge file in this brain. Each entry has enough detail to tell you whether you need to open the full file or not.
    2. The user will give you a task. Use the index to find relevant files, load only what you need, and get to work.

    ## How this system works

    - **You have no memory between sessions.** This brain is your memory. Everything you need to know is stored as `.md` files in three semantic spaces.
    ...
    │
    ├── identity/                ← WHO: SPECs + RULEs (design decisions, constraints)
    ├── knowledge/               ← WHAT: LEARNs + CODEs (discovered knowledge)
    │   └── indexes/
